...
export async function POST(
  ...
  try {
    ...
    // Validate sessionId length to match DB constraint (8-64 characters)
    const trimmedSessionId = sessionId.trim();
    if (trimmedSessionId.length < 8 || trimmedSessionId.length > 64) {
      return Response.json({ error: "Session ID must be between 8 and 64 characters" }, { status: 400 });
    }
    ...
    if (optionsError) {
      console.error("Error validating option IDs:", optionsError);
      return Response.json({ error: "Failed to validate option IDs" }, { status: 500 });
    }

    if (!validOptions || validOptions.length !== optionIds.length) {
      return Response.json({ error: "Invalid option IDs provided" }, { status: 400 });
    }
