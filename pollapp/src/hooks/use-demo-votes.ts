...
export function useDemoVotes(pollId: string) {
  ...
  // Generate sessionId once and persist it in localStorage
  const sessionId = useMemo(() => {
    const sessionKey = 'demo_vote_session_id';
    let storedSessionId = localStorage.getItem(sessionKey);
    
    if (!storedSessionId) {
      storedSessionId = generateBrowserFingerprint();
  ...
  // Memoize userVoteKey to prevent redeclaration on each render
  const userVoteKey = useMemo(() => `user_demo_vote_${pollId}`, [pollId]);
